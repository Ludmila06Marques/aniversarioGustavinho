...
function createFloorTile() {
    ...
    if (!ferryShown) {
        if (currentDecorationIndex < floorDecorations.length) {
            // Cria chão normal + decoração
            ...
            ferryImage = sceneInstance.add.image(tileX, tileY + 100, "ferry");
            ...
            ferryShown = true;
            nextFloorX += ferryImage.displayWidth; // Proxima posição será colada ao ferry
        }
    } 
    else if (ferryShown && ferryMeioCount > 0) {
        // Cria ferryMeio colado
        const meioImage = sceneInstance.add.image(tileX, tileY + 100, "ferryMeio");
        meioImage.setOrigin(0.5, 1);
        meioImage.setScrollFactor(1);
        meioImage.setDepth(1);
        meioImage.setScale(0.7);

        const meioBody = floorGroup.create(tileX, tileY, null)
            .setSize(meioImage.displayWidth, 45)
            .setVisible(false)
            .refreshBody();

        ferryMeioCount--;

        nextFloorX += meioImage.displayWidth * 0.7; // Ajusta pela escala
    }
}
